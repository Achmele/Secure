...
texte_10="Ouvrir"
texte_11="Selectionner un fichier "
texte_12="En cours"
texte_13="Ternimer"
...
def Translate():
    if config[2]:
        ...
        texte_9="Deverrouiller"
        texte_10="Ouvrir"
        texte_11="Selectionner un fichier "+mode[1]
        texte_12="En cours"
        texte_13="Ternimer"
        ...
        texte_9="Unlock"
        texte_10="Open"
        texte_11="Select an "+mode[1]+" file"
        texte_12="Process"
        texte_13="Ternimer"

    trans=[texte_0,texte_1,texte_2,texte_3,texte_4,texte_5,texte_6,texte_7,texte_8,texte_9,texte_10,texte_11,texte_12,texte_13]
    update_texte_x()
    return trans
def update_texte_x():
    lab9.config(text=trans[6]+"..")
    but5.config(text=trans[7])
    but6.config(text=trans[8])
    but7.config(text=trans[9])
    lab3.config(text=trans[11])
    print("bordel",trans[8])
...
def encrypt(key,file_in,file_out,run_monitor=False):
    ...
    with open (file_in,"rb") as src:
        ...
        with open(file_out,"wb") as out:
            ...
            if run_monitor :
                    ...
                    run_thread(txt_add,(texte_12,but5,.100,False),"5")
            while src.peek():
                b=ord(src.read(1))
                c=a%len(keys)
                d=bytes([b^keys[c]])
    ...
    if run_monitor:
        sleep(1)
        but5.config(text=trans[13],command=clean_all)
...
def next_2(file="bof....."):
    ...
    if file_path == "":
        ...
        lab8.config(text="")
        if mode[0]=="_lock":
            but5.config(image=img_lock,compound="right",text=trans[7],command=next_3)
        if mode[0]=="un_lock":
            but5.config(image=img_unlock,compound="right",text=trans[10],command=next_3)
...
def Go():
    ...
    run_thread(txt_add,(trans[7],lab1,.500,True,"blue"),"3")
...
but1=ttk.Button(screen_child1,text=trans[0],width=15,command=texte)
but2=ttk.Button(screen_child1,text=trans[1],width=15,command=image)
but3=ttk.Button(screen_child1,text=trans[2],width=15,command=audio)
but8=ttk.Button(screen_child1,text=trans[3],width=15,command=video )

       
lab3=ttk.Label(screen,text="",font=("bold",12))
but4=ttk.Button(screen,text=trans[4],command=lambda:next_2())
# pre_img=extrat_content("video.png")
# creat_file("mdr.png",pre_img,"wb")
...
put_entry=ttk.Label(screen,text=trans[5],font=("Italic",11))
put=ttk.Entry(screen,)
bar=ttk.Progressbar(length=300,mode="determinate",value=0)
lab9=ttk.Label(text=trans[6]+"..")
but5=ttk.Button(text=trans[7],style="warning",command=next_3)
but6=ttk.Button(screen_child2,text=trans[8],style="Custom1.TButton",width=10,command=mode_lock,image=lock_picture,compound="right")
but7=ttk.Button(screen_child2,text=trans[9],style="Custom1.TButton",width=10,command=mode_unlock,image=unlock_picture,compound="right")
lab10=ttk.Label(screen_child3,text="",background="#6e40c0")
off((but1,but2,but3,but8))

opening_img=resize_img("unlock_done_2.png",600,350)
lab11=ttk.Label(image=opening_img,text=trans[7],compound="top",style="warning",font=("Bold",13))
screen.after(1000,Go)
menu=ttk.Menu()
menu.add_command(label="Langague",command=change_langage)
